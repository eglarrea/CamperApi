...
public class AdminUserController {
	...
	@Operation(summary = "Obtener la lista de usuarios", description = "Devuelve la lista de usuarios de la empresa a la que pertenece el administrador. "
	...
	@GetMapping("/{id}")
	@PreAuthorize("hasRole('ADMIN')")
	@Operation(summary = "Obtener usuario por ID", description = "Este endpoint permite a un administrador obtener los datos de un usuario específico mediante su ID.", security = {
			@SecurityRequirement(name = "bearerAuth") }, parameters = {
					@Parameter(name = "Accept-Language", description = "Idioma de la respuesta (es, en, eu)", in = ParameterIn.HEADER, required = false) })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Usuario encontrado correctamente"),
